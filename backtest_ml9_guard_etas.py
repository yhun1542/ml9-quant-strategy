"""
ML9 + QV Ensemble Backtest with Guard + ETAS
Compare: No Guard, Guard Only, ETAS Only, Guard + ETAS
"""
...
from modules.etas_risk_manager import ETASRiskManager
...
def backtest_ensemble(
    ...
    etas: Optional[ETASRiskManager] = None,
) -> tuple[pd.Series, dict]:
    """
    Backtest ML9 + QV ensemble
    Apply Guard and/or ETAS if provided
    """
    # Align dates
    df = pd.concat(
    ...
    if guard is not None:
        ml9_scales_guard = guard.get_scale_series(df.index)
    else:
        ml9_scales_guard = pd.Series(1.0, index=df.index)

    # Apply ETAS
    if etas is not None:
        etas_scales = etas.get_scale_series(df.index)
    else:
        etas_scales = pd.Series(1.0, index=df.index)

    # Combined scale: multiply Guard and ETAS
    combined_scale = ml9_scales_guard * etas_scales
    df["ml9_scaled"] = df["ml9"] * combined_scale
...
def main():
    print("="*100)
    print("ML9 + QV ENSEMBLE BACKTEST: GUARD + ETAS COMPARISON")
    print("="*100)
    ...
    # 3) Initialize ETAS
    print("Initializing ETAS Risk Manager...")
    etas_config = {
        "enabled": True,
    ...
    etas = ETASRiskManager(etas_config)
    etas.initialize(spx)
    
    # 4) Backtest scenarios
    scenarios = [
        ("No Guard, No ETAS", None, None),
        ("Guard Only", guard, None),
        ("ETAS Only", None, etas),
        ("Guard + ETAS", guard, etas),
    ]
    
    results = {}
    
    for name, g, e in scenarios:
        print("\n" + "="*100)
        print(f"{name}")
        print("="*100)
        
        ret, metrics = backtest_ensemble(ret_ml9, ret_qv, w_ml9=0.6, w_qv=0.4, guard=g, etas=e)
        ...
        results[name] = {
            "metrics": metrics,
            "daily_returns": {
                "index": ret.index.strftime("%Y-%m-%d").tolist(),
                "values": ret.tolist(),
            },
        }
    ...
    print(f"\n{'Scenario':<20} {'Sharpe':>8} {'Ann Ret':>10} {'Ann Vol':>10} {'Max DD':>10}")
    print("-" * 60)
    for name in ["No Guard, No ETAS", "Guard Only", "ETAS Only", "Guard + ETAS"]:
        m = results[name]["metrics"]
        print(f"{name:<20} {m['sharpe']:>8.2f} {m['annual_return']*100:>9.2f}% "
              f"{m['annual_vol']*100:>9.2f}% {m['max_dd']*100:>9.2f}%")
    
    # 6) Save results
    out_data = {
        "scenarios": results,
        "guard_config": guard_config,
        "etas_config": etas_config,
    }
    
    out_path = BASE_DIR / "results" / "ml9_guard_etas_comparison.json"
