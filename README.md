...
**⚠️ 중요**: 이 리포지토리는 코드만 포함하고 있습니다. 데이터 파일은 별도로 제공됩니다.

**옵션 A**: 전체 프로젝트 다운로드 (코드 + 데이터, 468MB)
```bash
# 전체 압축 파일 다운로드 (별도 제공)
# ml9-quant-strategy-complete.tar.gz
tar -xzf ml9-quant-strategy-complete.tar.gz
```

**옵션 B**: Polygon API로 직접 다운로드
```bash
# Polygon API 키 설정 필요
...
- **docs/ML9_ROBUSTNESS_REPORT.md**: 종합 강건성 테스트 리포트
- **docs/V1_5_FINAL_REPORT.md**: v1.5 거래비용 포함 최종 리포트
- **docs/ML9_SIGNAL_SHUFFLE_REPORT.md**: 신호 검증 테스트 리포트
